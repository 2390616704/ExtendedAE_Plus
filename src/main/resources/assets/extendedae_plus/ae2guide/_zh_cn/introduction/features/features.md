...
- **搜索同步快捷键（默认 F）：**  将 JEI 中的物品名称同步至 AE2 搜索栏与 ExtendedAE 扩展样板管理界面；可在键位设置中自定义。  
- **样板优先匹配：**  编写样板时优先匹配 JEI 书签靠前的物品。


### 3、Ctrl+Q 配方书签样板创建

- **合成/切石机/锻造台样板：** 会直接尝试通过玩家可访问的 AE 网络上传到装配矩阵；该流程不需要打开样板编码终端界面。
- **处理样板：** 会直接打开样板供应器选择界面，并按现有配方类型映射规则自动填入搜索关键词。
- **特殊书签不进背包：** 这条特殊流程下，生成的样板不会放入玩家背包；即使上传失败或选择界面无法打开，也不会额外生成背包样板。
- **普通书签行为：** 普通 JEI 书签会直接生成样板并放入背包；会自动检索配方，且配方选择时优先合成样板。
